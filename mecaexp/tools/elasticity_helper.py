...
def transverse_elasticity(young_p: float, young_z: float, poisson_p: float, poisson_pz: float, shear_zp: float) -> np.ndarray:
    ...
    ret[0, 0, 0, 0] = (1 - poisson_pz * poisson_zp) / (young_p * young_z * denominator)
    ret[1, 1, 1, 1] = (1 - poisson_pz * poisson_zp) / (young_p * young_z * denominator)
    ret[2, 2, 2, 2] = (1 - pow(poisson_p, 2.)) / (pow(young_p, 2.) * denominator)
